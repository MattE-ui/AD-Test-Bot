...
class CountingGame(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.current_count % 100 == 0:
            await message.channel.send(f"🎉 We've reached **{self.current_count}**! Keep it going!")
            await message.add_reaction("🎉")
    ...
    async def pause_counting(self, interaction: discord.Interaction):
        ...
        _save_config(self._config)
        await interaction.response.send_message(f"⏸️ Counting game paused. Current count: **{self._config['current_count']}**.")
    ...
    async def restore_count(self, interaction: discord.Interaction, number: int):
        ...
        _save_config(self._config)
        await interaction.response.send_message(f"✅ Count set to **{number}**. Next expected: {number + 1}.")
    ...
    async def skip_to_count(self, interaction: discord.Interaction, number: int):
        ...
        if number < current + 1:
            await interaction.response.send_message(f"❌ Cannot skip to less than {current + 1}. Current: {current}.", ephemeral=True)
            return

        self._config["current_count"] = number
        _save_config(self._config)
        await interaction.response.send_message(f"✅ Skipped to **{number}**. Next expected: {number + 1}.")

    @app_commands.command(name="view_stats", description="View counting stats (Admin only).")
